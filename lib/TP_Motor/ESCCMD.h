...
#define ESCCMD_CMD_REPETITION   25//10                // Number of time commands have to be repeated to be acknowledged by ESC
...
int DSHOT_arm(){
  ...
  // Define stop command
  for ( i = 0; i < ESCCMD_n; i++ )  {
    ESCCMD_cmd[i] = DSHOT_CMD_BEACON2;
    ESCCMD_tlm[i] = 0;
  }
...
int ESCCMD_beep( void )  {
  static int i, k;

  // Define BEEP2 on command
  for ( i = 0; i < ESCCMD_n; i++ )  {
    ESCCMD_cmd[i] = 6;
    ESCCMD_tlm[i] = 0;
  }
