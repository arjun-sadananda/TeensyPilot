...
#define constrain_ftype(amt, low, high) constrain_value(ftype(amt), ftype(low), ftype(high))
...
// degrees -> radians
static inline constexpr ftype AP_radians(ftype deg)
{
    return deg * AP_DEG_TO_RAD;
}

// radians -> degrees
static inline constexpr float AP_degrees(float rad)
{
    return rad * AP_RAD_TO_DEG;
}

template<typename T>
ftype AP_sq(const T val)
{
    ftype v = static_cast<ftype>(val);
    return v*v;
}
static inline constexpr float AP_sq(const float val)
{
    return val*val;
}
...
ftype AP_sq(const T first, const Params... parameters)
{
    return AP_sq(first) + AP_sq(parameters...);
}
...
{
    return sqrtF(AP_sq(first, second, parameters...));
}
