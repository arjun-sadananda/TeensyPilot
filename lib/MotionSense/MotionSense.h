...
#define G_PER_COUNT            0.00024414062f  // = 1/4096
#define DEG_PER_SEC_PER_COUNT  0.01526717557f  // = 1/65.5
#define UT_PER_COUNT           0.1f
...
public:
	...
	void readMotionSensor(int& ax, int& ay, int& az, int& gx, int& gy, int& gz, int& mx, int& my, int& mz) {
		if (!newdata) update();
		newdata = 0;
		gx = 0;//gyro_accel_raw[0];
		gy = 0;//gyro_accel_raw[1];
		gz = 0;//gyro_accel_raw[2];
		ax = 0;//gyro_accel_raw[3];
		ay = 0;//gyro_accel_raw[4];
		az = 0;//gyro_accel_raw[5];
		mx = mag_raw[0];
		my = mag_raw[1];
		mz = mag_raw[2];
	}
	void readMotionSensor(float& ax, float& ay, float& az, float& gx, float& gy, float& gz) {
		if (!newdata) update();
		newdata = 0;
		ax = (float)gyro_accel_raw[0] * G_PER_COUNT;
		ay = (float)gyro_accel_raw[1] * G_PER_COUNT;
		az = (float)gyro_accel_raw[2] * G_PER_COUNT;
		gx = (float)mag_raw[0] * DEG_PER_SEC_PER_COUNT;
		gy = (float)mag_raw[1] * DEG_PER_SEC_PER_COUNT;
		gz = (float)mag_raw[2] * DEG_PER_SEC_PER_COUNT;
	}
	void readMotionSensor(float& ax, float& ay, float& az, float& gx, float& gy, float& gz, float& mx, float& my, float& mz) {
		if (!newdata) update();
		newdata = 0;
		ax = 0;//(float)gyro_accel_raw[3] * G_PER_COUNT - cal[0];
		ay = 0;//(float)gyro_accel_raw[4] * G_PER_COUNT - cal[1];
		az = 0;//(float)gyro_accel_raw[5] * G_PER_COUNT - cal[2];
		gx = 0;//(float)gyro_accel_raw[0] * DEG_PER_SEC_PER_COUNT - cal[3];
		gy = 0;//(float)gyro_accel_raw[1] * DEG_PER_SEC_PER_COUNT - cal[4];
		gz = 0;//(float)gyro_accel_raw[2] * DEG_PER_SEC_PER_COUNT - cal[5];
		float x = (float)mag_raw[0];//-456.95;// * UT_PER_COUNT - cal[6];
		float y = (float)mag_raw[1];//-284.5;// * UT_PER_COUNT - cal[7];
		float z = (float)mag_raw[2];//-72.2;// * UT_PER_COUNT - cal[8];
		mx = x;//x * cal[10] + y * cal[13] + z * cal[14];
		my = y;//x * cal[13] + y * cal[11] + z * cal[15];
		mz = z;//x * cal[14] + y * cal[15] + z * cal[12];
	}
...
private:
	...
	bool MPU6050_begin();
	bool BMP280_begin();
	bool MPU_read(int16_t *data);
	bool MAG_read(int16_t *data);
	bool BMP280_read(int32_t *altitude, int16_t *temperature);
	float cal[16]; // 0-8=offsets, 9=field strength, 10-15=soft iron map
	int16_t gyro_accel_raw[6];
	int16_t mag_raw[3];
