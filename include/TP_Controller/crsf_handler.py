...
class CRSFHandler:
    ...
    def __init__(self, port='COM19', baudrate=921600):
        try:
            # write_timeout=0 prevents the code from hanging if the serial port fails
    ...
    def write_stabilize(self, roll, pitch, throttle, yaw_rate):
        if not self.initialized:
            return
        self.ser.write(channelsCrsfToChannelsPacket([roll, pitch, throttle, yaw_rate] + [992 for ch in range(12)]))
    ...
    def write(self, channels):
        if not self.initialized:
            return
        self.ser.write(channelsCrsfToChannelsPacket(channels))
        # print("Sent CHANNELS_PACKED")
        time.sleep(0.020)
...
"""
    def readStatus_write(roll, pitch, throttle, yaw_rate):
        if ser.in_waiting > 0:
